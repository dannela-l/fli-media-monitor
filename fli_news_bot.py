...
def clean_date(date_string):
    """Convert UTC-style timestamp into Pacific Time when possible."""
    try:
...
def normalize_url(url):
    """Strip query params and trailing slash to improve deduping."""
    if not url:
        return ""
...
def classify_article(headline, summary):
    ...
    - only checks headline + summary
    - exact phrase match for FLI terms and spokesperson full names
    """
...
def main():
    ...
    if not formatted_fli and not formatted_relevant:
        print("No matching top-tier relevant articles found.")
        return
    ...
    print(
        f"Posted threaded clipbook with {len(formatted_fli)} Future of Life Institute "
        f"article(s) and {len(formatted_relevant)} Relevant Coverage article(s)."
    )
